...
bool PrimeNetClient::register_machine() {
    ...
    std::string url = "https://v5.mersenne.org/v5server/"
        ...
        "&hg=" + hw_guid +
        "&wg="
        "&a=" + url_encode("macOS,PrimePath," + PrimePathVersionUTF8() +
                          " (github.com/s1rj1n/primepath)") +
...
std::string PrimeNetClient::build_result_json(const TFResult& result) {
    ...
    js << ",\"program\":{\"name\":\"PrimePath\",\"version\":\"" << PrimePathVersionUTF8()
       << "\",\"kernel\":\"Metal96bit\"}"
    ...
    // program.details;os.os;os.architecture;timestamp
    {
        ...
        ck << ";"
           ...
           << "PrimePath;" << PrimePathVersionUTF8() << ";Metal96bit;"
...
std::string PrimeNetClient::machine_description() {
    ...
    desc << "PrimePath/" << PrimePathVersionUTF8() << " | " << chip_str
         << " | " << ncpu << " CPU";
    if (perf_cores > 0)
