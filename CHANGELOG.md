...
- **Version number in title bar and status header** -- the main window and status pane still reported v0.5, several releases out of date ([#2](https://github.com/s1rj1n/primepath/issues/2)).
- **Undefined behaviour in Pollard p-1 logging** -- the factor line formatted an `NSString *` with `%s`, reading the object pointer as a C string. Triggered whenever p-1 actually found a factor.

### Changed
- Version bumped to 1.4.1 across Info.plist, User-Agent, PrimeNet registration, JSON output, window title, and status header.
...
### New
- **Mersenne TF checkpoint file** (`mersenne_tf_checkpoint_M{exponent}.txt`) -- written every 30 seconds with all fields AutoPrimeNet needs for progress reporting: exponent, bit range (bit_lo/bit_hi), current k-value, tested count, cumulative elapsed time, assignment key, and timestamp. AutoPrimeNet reads this to calculate percentage, speed, and ETA.
...
- **JSON sample button** now outputs a complete sample checkpoint file with field descriptions and progress calculation formulas.

### Fixed
- **Resume detection** -- Start TF now keeps the saved k-position, elapsed time, and tested count when resuming the same assignment, instead of restarting the range.
- **`start_pos` not loaded** from `search_progress.txt` on launch.

### Changed
- **Per-exponent checkpoint filename** -- the checkpoint is written as `mersenne_tf_checkpoint_M{exponent}.txt`, per AutoPrimeNet recommendation, so concurrent exponents no longer overwrite one another.
- Help text and data file listings updated to document the checkpoint file.
